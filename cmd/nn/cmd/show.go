...
var virtualGlobalProtocols = []virtualProtocol{
	...
	{
		ID:          "virtual-nn-transcript-integration-receipts",
		Title:       "Protocol: create transcript integration receipts after substantive adoption",
		AppliesWhen: "after parent inspection consequentially adopts, partially adopts, or rejects substantive delegated work and retained transcript evidence is available",
		Body:        "After inspecting attributable retained transcript evidence, create one transcript integration receipt only when the parent consequentially adopted, partially adopted, or rejected substantive delegated work. Run `nn transcript receipt <session> --assignment \"...\" --disposition accepted|partially-adopted|rejected` and record supplied adopted, rejected, result, and verification facts. Link it to an existing task, decision, or result note when that relationship exists. Do not create a receipt for every subagent return, routine lookup, duplicate verifier, or aborted run with no consequential adjudication. Do not copy raw transcript bodies or sensitive prompts into the notebook. The receipt is an expiring draft and does not turn producer output into accepted truth; its disposition records the parent-side decision.\n",
	},
	{
		...
		Body: "**Anti-skip-resistance rule:** Any assistant text that cites prior knowledge, familiarity, or certainty about a topic as a reason not to run the gate search is itself a gate violation — prior knowledge is the condition under which the search is *most* likely to return a relevant note, not a reason to skip it. An evaluator detects this violation by finding phrases such as \"I already know,\" \"I'm familiar with,\" \"no need to search,\" or \"I know where this is\" in assistant text immediately before a gated action that has no `Gate:` line.\n\n" +
			"This protocol does not govern direct structured tools that do not perform those actions. Do not substitute an nn retrieval command for another available tool that directly observes the requested relation; choose the tool whose contract supplies the requested evidence.\n\n" +
			...
			"- `nn trace <root-dir> --symbol <name> [--root <dir>]` or `nn trace <file>:<line>` — an allow-listed syntax-aware option for tracing call graphs or symbol relationships across files; add `--root <dir>` to index a wider directory (e.g. the repo root) so cross-package calls resolve; the call itself satisfies the gate\n" +
			...
			"**Staleness check (runs after any `nn show` in the gate window, independent of compare outcome):** Expiry keys on *claim volatility*, not concreteness — the question is: could a future change make this note's asserted claim false or unsafe to rely on? Concreteness is a poor proxy: a source- and date-bounded fact is concrete yet permanently true. Classify the stored claim exactly as written in the note body, including its provenance qualifiers. Do not strengthen, reinterpret, or replace it using the title, freshness display, neighboring notes, search context, or the current conversation. Before writing any `stale:` line, quote `classified proposition: \"<verbatim sentence from the note body>\"`. Then classify that proposition into exactly one of three categories and write the matching `stale:` line:\n" +
			"- **Historical observation** — the claim is explicitly bounded to a past source, event, commit, version, or date (e.g. \"the March 4 RFP stated X\", \"commit abc123 implemented Y\"). If the body explicitly anchors the claim to a past source, commit, revision, event, or date, stop immediately: classify it as historical and never add expiry. It stays true because it is anchored to that source/time. Freshness may limit using that observation as evidence about the present, but it does not make the historical observation expire. Write `stale: historical — bounded to past source, retain` and take no expiry action, regardless of age.\n" +
	...
	},
	{
		...
		Body: "**nn new** `--title \"...\" --type <type> --content \"...\" --no-edit [--tags \"t1,t2\"] [--aliases \"TDD,ACR\"] [--link-to <id> --link-type <type> --annotation \"...\"] [--applies-when \"...\"] [--representation <rep>] [--expires YYYY-MM-DD] [--expires-when \"condition\"] [--check]`\n" +
			...
			"--link-to, --link-type, and --annotation are repeatable and must be paired positionally; --check runs representation graph validation after update (no-op if note has no representation).\n\n" +
			...
			"One-shot relationship workflow: `search → select anchor → nn graph show --focus <id> --depth 1 --direction both --zones → nn graph bodies with the identical traversal → retrieve every page under the page-1 snapshot → reconstruct all selected bodies → answer`. Topology arrives first, but do not make body-derived claims until every page and ordered segment is complete. When a neighbor shows an unusually high degree (`↑out ↓in` markers), treat it as a likely hub worth inspecting even if off the direct question — high inbound degree signals a load-bearing note.\n\n" +
			"**Transcript exploration / Transcript Office — binding skill dispatch.** For agent runs, subagent transcripts, recent work, assignments, or execution-pattern discovery, run `nn skills list` if not yet run this session, then `nn skills get nn-transcript`. Follow its lazy dispatch: load each applicable owner with `nn skills get nn-transcript --reference <name>` before acting. Transcript navigation belongs to `nn-transcript`; notebook-graph navigation belongs to `nn-navigate`. Detailed workflows and command flags remain in the skills.\n\n" +
			"**nn transcript receipt** `<session> --assignment TEXT --disposition accepted|partially-adopted|rejected [--parent-session ID] [--adopted TEXT] [--rejected TEXT] [--result TEXT] [--verification TEXT] [--expires-in DURATION] [--link-to ID --link-type TYPE --annotation TEXT]` — create one expiring draft observation after parent-side inspection and consequential adjudication of substantive delegated work; repeat evidence/result flags and link triples as needed; records stable session identity without copying raw transcript bodies.\n\n" +
			"**Human-driven iterative navigation — binding skill dispatch.** Before notebook-graph navigation — including a positioned multi-turn walk or requests to teleport, orient, recenter, peek, scan, arrive, use Back/Forward history, or manage bookmarks — if you have not yet done so this session, run `nn skills list`; then run `nn skills get nn-navigate` before navigating. Its compact core requires fetching each applicable owning reference with `nn skills get nn-navigate --reference <name>` before that action. `nn-navigate` is the sole owner of the detailed workflow, presentation contract, colon-prefixed conversational DSL with conversation-scoped Guided/Advanced interaction, and conversational navigation state; those intents are not `nn` subcommands. nn-navigate enforces the color relay discipline for human-facing views. nn-navigate owns Find an analog and the optional Analogize, Find gaps, Visualize, and Quiz lenses. This virtual protocol retains only the summary dispatch and command reference.\n\n" +
			"Default neighborhood: `nn graph show --focus <id> --depth 1 --direction both --zones --format text` — `--zones` groups the anchor's direct neighbors under TOP (what it answers to), BOTTOM (what builds on it), LEFT (tension: contradicts/questions), RIGHT (lateral: source-of/requires/follows), and UNCLASSIFIED (legacy untyped relationships with no inferred navigation semantics). Use `--depth 1` with `--zones`: only direct neighbors carry a category, and the zoned text view omits uncategorized nodes — so for the full 2-hop neighborhood use `--depth 2` *without* `--zones`. Then narrower helpers as needed: incoming influence/use → `nn backlinks <id>`; outgoing endorsed claims/evidence → `nn links <id>`; connected note bodies → `nn show <id> --depth 2`; relationship between two anchors → `nn path <a> <b>`; exact local region → `nn clusters --focus ID --json`; exact local bridge status → `nn graph bridges --focus ID --format json`; global topic-level structure/hubs → `nn clusters`; notes that connect otherwise-disconnected parts of the graph (integration points / load-bearing links between topic areas) → `nn graph bridges`, or a query-conditioned bridge lookup → `nn graph bridges --search \"<query>\" --format json`; potentially missing edges → `nn list --similar <id>`. Use `--direction outgoing` for what grounds or follows from the anchor, `--direction incoming` for who relies on/extends/operationalizes it, `both` when exploratory.\n\n" +
			...
			"**nn link** `<from> <to> --type <type> --annotation \"...\"` — type must be canonical and non-empty\n" +
			"**nn link set-type** `<from> <to> --type <type> [--annotation-matches \"fragment\"]` — atomically type one legacy untyped relationship; rejects ambiguous matches and preserves annotation/endpoints\n" +
			...
			"**nn grep** `<pattern> [path] [-i|--ignore-case] [--context N] [--notes-per-match K] [--max-matches N] [--trace [--intent TEXT]] [--context-report]` — search files for pattern and annotate each match with related nn notes via BM25 on surrounding context; use instead of bare grep when working in a codebase to surface relevant knowledge alongside code locations; `-i`/`--ignore-case` makes matching case-insensitive; `--trace` invokes nn trace inline for each traceable matched file; `--context-report` appends retained source-window counts and exact gross, unique, and overlap line totals\n\n" +
			"**nn trace** `<root-dir> --symbol <name> [--symbol <name> ...] [--depth N] [--json] [--show-unresolved] [--root <dir>] [--intent TEXT]` or `<file>:<line>` — syntax-aware call graph from entry-point symbols using gotreesitter; unlike `nn grep`, traverses inferred call relationships rather than text matches, but project-local resolution may be unresolved or ambiguous; `--intent` affects related-note ranking only and should describe a purposeful agent investigation; annotates each resolved node with related nn notes via BM25; emits human-readable tree by default or JSON graph with --json; `file:line` form resolves the symbol at that line automatically; `--root <dir>` widens the index scope (language-agnostic) so calls to definitions in sibling packages resolve instead of showing as unresolved — pass the repo/project root to follow logic across packages\n\n" +
			...
			"**nn media** — media extraction never writes the notebook. `nn media prepare <file> [--every DURATION] [--frames TIMESTAMPS] [--contact-sheet] [--engine NAME] [--json]` combines requested stages in one durable run. `nn media context --run RUN_ID [--max-bytes N] [--page N]` retrieves bounded `nn.media.context/v1` without reprocessing, preserving run/source/bundle/manifest provenance, qualified coverage, transcript chunks, typed readable image path+MIME+timestamp attachments, and truncation/next-page disclosure. `nn media runs RUN_ID [--json]` remains discovery. Standalone `nn media inspect`, `nn media sample`, `nn media transcribe`, and `nn media doctor` remain available and non-TTY-safe. No media subcommand mutates notebook truth and no notebook write occurs. There is no `nn integrate` command. Natural `integrate this media run RUN_ID` dispatches to nn-navigate for image loading, notebook search, and a non-mutating approval proposal. Experimental Parakeet remains explicit.\n\n" +
			...
			"**nn path** `<id-a> <id-b> [--links TYPE,...] [--json]` — shortest undirected path by default with legacy text and JSON unchanged; `--links` follows stored direction and returns `{witnesses:[{nodes,edges}]}` with at most 3 shortest typed witnesses, selected by distinct first-hop nodes, then edge type-sequence diversity, then lexical full-path order\n\n" +
			"**nn graph bridges** `nn graph bridges [--search QUERY] [--format text|json] [--exclude ID] [--limit N]` or `nn graph bridges --focus ID --format json` — return one unified rich bridge model in either encoding. Focus mode returns `{focus,bridge}` for the exact known note, bypasses ranking/default limit, rejects search/exclude/explicit limit, fails for an unknown ID, and returns `bridge: null` for a known non-bridge; the nested bridge is the same rich record. Every result has `id`, `title`, structural `score`, `relevance_score`, and deterministic `witnesses` (never singular `witness`) containing at most 3 crossing examples. The command sorts all incoming and outgoing edges by endpoint ID, type, and annotation, forms edge pairs, keeps the earliest edge pair for each ephemeral full-graph region pair, orders pairs by incoming then outgoing representative ID (using an internal stable unclustered sentinel plus endpoint ID), and applies the cap after deduplication. Each crossing carries both edges plus bounded `regions` context from full-graph label propagation: endpoint summaries (`representative` and full `size`) or unclustered, and `same_region`; text labels them `crossing 1` through `crossing N`. Without search, relevance is JSON `null` / text `n/a`; search supplies normalized numeric relevance and relevance-first ranking. No durable region IDs are exposed, same-region pairs remain eligible, and a crossing is not proof of territorial separation. The existing non-focus text/JSON schemas, ranking, filtering, and limits remain unchanged.\n\n" +
			"**nn graph routes** `nn graph routes --focus ID --links TYPES --search QUERY --limit N --json [--explain]` — discover query-relevant destinations reachable from the focus through only the selected typed, directed links; destination ranking is unchanged and every entry has `destination` plus `witnesses` containing at most 3 shortest node-and-edge paths selected by first-hop diversity, edge type-sequence diversity, then lexical full-path order. Without `--explain`, output remains a top-level route array; `--explain` requires `--json` and wraps it as `{routes, diagnostics}` with aggregate `query_tokens`, `total_notes`, `direct_lexical_matches`, `focus_excluded`, `typed_reachable`, `eligible_destinations`, separate annotation-aware `graph_scored_matches`, `returned`, and `truncated_by_limit`—never note content or candidate dumps.\n\n" +
			"**nn graph impact** `nn graph impact --focus ID --links TYPES --direction incoming|outgoing --depth N --json` — return the bounded typed impact set ordered exactly as before. Top-level `summary` contains `total_impacts`, depth-ascending `counts_by_depth`, count-descending-then-ID `counts_by_first_hop` (selected-witness branches may overlap), and honest `witnesses_truncated`; each `node`/`depth` entry still carries unchanged `witnesses` containing at most 3 diverse shortest paths. Outgoing follows stored source→target links, while incoming witness nodes run focus→impact and every edge retains stored source→target orientation.\n\n" +
			"**nn clusters** `nn clusters [--min N] [--singletons] [--json]`, `nn clusters --search QUERY --json [--summary] [--match-limit N]`, or `nn clusters --focus ID --json [--min N] [--singletons]` — topological clusters via label propagation. Focus mode returns `{focus,cluster}` for the exact known note using the same full membership and representative; active min/singleton omission returns `cluster: null`, unknown IDs fail, and search/summary/match-limit are rejected. Search JSON ranks clusters by the sum of their top three normalized match scores and includes exhaustive `match_count`, `match_density` = `match_count / size` as an explanatory signal rather than a ranking input, `score`, `representative`, ranked `matches`, and full `notes` outside summary mode. `--summary` omits `notes`; its summary-only `--match-limit N` defaults to 3 matches per cluster, 0 means all, and negative values fail. Summary always emits `matches_returned` and `matches_truncated` (including false); match truncation occurs only after exhaustive count, density, score, cluster ranking, and representative computation, and never limits the number of clusters. Non-summary JSON has no match-limit metadata and remains exhaustive. The existing non-focus text/JSON schemas, search projection, filtering, and ordering remain unchanged.\n\n" +
			...
			"**nn graph show** `[--focus <id>] [--depth N] [--direction outgoing|incoming|both] [--links TYPE,...] [--status STATUS,...] [--representation VALUE] [--zones] [--presentation-hints] [--color auto|always|never] [--format text|json|mermaid]` — traversal-filtered graph output (LLM-facing; focused traversal defaults to depth 2 and direction outgoing); link, status, and representation filters constrain BFS expansion rather than hiding nodes after traversal; Mermaid preserves stored edge orientation and emits deterministic provenance comments. Without --focus, graph show renders the full graph; explicitly supplied traversal flags require `--focus`. `--zones` (requires `--focus`) annotates each node with the directional screen zone it occupies relative to the focus, derived from its direct link to the focus: TOP = what the focus answers to (governs/supports incoming, or refines/extends/grounded-by outgoing from focus); BOTTOM = what builds on the focus (governs/supports outgoing, or refines/extends/grounded-by incoming); LEFT = tension (contradicts, questions); RIGHT = lateral provenance/task/process-history edges (source-of, requires, follows); UNCLASSIFIED = directly connected legacy links with an empty type, without inferred directional semantics. In `--format json` each node gains a `zone` field (omitted when empty); in `--format text` nodes are grouped under TOP/LEFT/RIGHT/BOTTOM/UNCLASSIFIED headers, preceded by a zone→link-type key. This is the same zone mapping the interactive HTML viewer uses. Every node carries a link-degree marker: `--format text` appends `↑<out> ↓<in>` to each node line; `--format json` adds `out_degree`/`in_degree` fields (degree is over the whole notebook, so it distinguishes a hub from a leaf). `--color auto|always|never` (default auto) prefixes text output with colored-circle emoji markers (chosen over ANSI so they survive markdown/relay): zone headers by zone, node titles by note type, edge labels by link family (🔴 tension/🔵 lateral/🟦 structural) — auto emits markers only to a TTY (piped output and json stay clean), always forces them (use when relaying to a human), never suppresses them. `--presentation-hints` adds degree-based LLM relay budgets to topology. The compatibility flag `--bodies` is deprecated and hidden from normal flag help; using it warns on stderr while preserving the exact existing complete, unpaginated stdout. Use `nn graph bodies` for new body retrieval.\n\n" +
			"**nn graph bodies** `[--focus <id>] [--depth N] [--direction outgoing|incoming|both] [--links TYPE,...] [--status STATUS,...] [--representation VALUE] [--page N] [--snapshot SHA256]` — lossless compact JSON body transport for the same filtered traversal set as graph show. Page 1 returns a repeated 64-hex snapshot, total pages, next page, and ID-keyed ordered UTF-8 body segments. Retrieve every page; page 2 and later require the page-1 snapshot, and a notebook or traversal change rejects it as stale or mismatched. Every encoded page is at most 48,000 bytes including JSON overhead. Huge bodies span segments and empty bodies have one explicit empty segment. Concatenate decoded segments in order for exact body bytes, and make no body-derived claims until the complete batch reconstructs. The metadata boundary is strict: records contain only ID, segment ordinals, and stored body fragments—not frontmatter, links, topology, relay hints, or commentary.\n\n" +
			...
			"For the full command reference, run `nn skills get nn-guide`.\n",
	},
	{
	...
	},
	{
		...
		Body: "If you have not yet run `nn skills list` this session, run it before any `nn skills get` " +
			...
			"nn skills get <name>\n" +
			"```\n\n" +
			"When that compact core dispatches an action to a lazy reference, fetch the named owner before acting; " +
			"use `nn skills get <name> --list-references` to inspect applicability and " +
			"`nn skills get <name> --reference <reference-name>` to retrieve it.\n",
	},
}
...
func freshnessLabel(modified time.Time) string {
	...
	default:
		return "stale (" + ageStr + " — revalidate before using as evidence of current conditions; this does not imply the historical note should expire)"
	}
}
