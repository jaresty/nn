...
var virtualGlobalProtocols = []virtualProtocol{
	...
	},
	{
		...
		Body: "**nn new** `--title \"...\" --type <type> --content \"...\" --no-edit [--tags \"t1,t2\"] [--link-to <id> --annotation \"...\"] [--applies-when \"...\"] [--representation <rep>] [--expires YYYY-MM-DD] [--expires-when \"condition\"] [--check]`\n" +
			...
			"**nn graph routes** `nn graph routes --focus ID --links TYPES --search QUERY --limit N --json [--explain]` — discover query-relevant destinations reachable from the focus through only the selected typed, directed links; returns ranked destinations with aligned shortest node-and-edge witnesses for Orient, Scan, Peek, Recenter, and Arrive. Without `--explain`, output remains the legacy route array; `--explain` requires `--json` and wraps it as `{routes, diagnostics}` with aggregate `query_tokens`, `total_notes`, `direct_lexical_matches`, `focus_excluded`, `typed_reachable`, `eligible_destinations`, separate annotation-aware `graph_scored_matches`, `returned`, and `truncated_by_limit`—never note content or candidate dumps.\n\n" +
			"**nn graph impact** `nn graph impact --focus ID --links TYPES --direction incoming|outgoing --depth N --json` — return the bounded typed impact set with one deterministic shortest witness per reached note; outgoing follows stored source→target links, while incoming traverses in reverse but keeps witness edges in stored orientation for Scan, Peek, Recenter, and Arrive\n\n" +
