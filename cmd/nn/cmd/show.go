...
var virtualGlobalProtocols = []virtualProtocol{
	...
	},
	{
		...
		Body: "**nn new** `--title \"...\" --type <type> --content \"...\" --no-edit [--tags \"t1,t2\"] [--aliases \"TDD,ACR\"] [--link-to <id> --link-type <type> --annotation \"...\"] [--applies-when \"...\"] [--representation <rep>] [--expires YYYY-MM-DD] [--expires-when \"condition\"] [--check]`\n" +
			...
			"**nn trace** `<root-dir> --symbol <name> [--symbol <name> ...] [--depth N] [--json] [--show-unresolved] [--root <dir>]` or `<file>:<line>` — syntax-aware call graph from entry-point symbols using gotreesitter; unlike `nn grep`, traverses inferred call relationships rather than text matches, but project-local resolution may be unresolved or ambiguous; annotates each resolved node with related nn notes via BM25; emits human-readable tree by default or JSON graph with --json; `file:line` form resolves the symbol at that line automatically; `--root <dir>` widens the index scope (language-agnostic) so calls to definitions in sibling packages resolve instead of showing as unresolved — pass the repo/project root to follow logic across packages\n\n" +
