...
var virtualGlobalProtocols = []virtualProtocol{
	...
	},
	{
		...
		Body: "**nn new** `--title \"...\" --type <type> --content \"...\" --no-edit [--tags \"t1,t2\"] [--link-to <id> --link-type <type> --annotation \"...\"] [--applies-when \"...\"] [--representation <rep>] [--expires YYYY-MM-DD] [--expires-when \"condition\"] [--check]`\n" +
			...
			"**nn graph bridges** `nn graph bridges [--search QUERY] [--format text|json] [--exclude ID] [--limit N]` — return one unified rich bridge model in either encoding. Every result has `id`, `title`, structural `score`, `relevance_score`, and deterministic `witnesses` (never singular `witness`) containing at most 3 crossing examples. The command sorts all incoming and outgoing edges by endpoint ID, type, and annotation, forms edge pairs, keeps the earliest edge pair for each ephemeral full-graph region pair, orders pairs by incoming then outgoing representative ID (using an internal stable unclustered sentinel plus endpoint ID), and applies the cap after deduplication. Each crossing carries both edges plus bounded `regions` context from full-graph label propagation: endpoint summaries (`representative` and full `size`) or unclustered, and `same_region`; text labels them `crossing 1` through `crossing N`. Without search, relevance is JSON `null` / text `n/a`; search supplies normalized numeric relevance and relevance-first ranking. No durable region IDs are exposed, same-region pairs remain eligible, and a crossing is not proof of territorial separation.\n\n" +
