...
var virtualGlobalProtocols = []virtualProtocol{
	...
	},
	{
		...
		Body: "**nn new** `--title \"...\" --type <type> --content \"...\" --no-edit [--tags \"t1,t2\"] [--link-to <id> --annotation \"...\"] [--applies-when \"...\"] [--representation <rep>] [--expires YYYY-MM-DD] [--expires-when \"condition\"] [--check]`\n" +
			...
			"Workflow: `search → select anchor → nn graph show --focus <id> --depth 1 --direction both --zones → inspect key note bodies → answer`.\n" +
			...
			"**nn graph show** `[--focus <id>] [--depth N] [--direction outgoing|incoming|both] [--links TYPE,...] [--status STATUS,...] [--representation VALUE] [--zones] [--bodies] [--format text|json|mermaid]` — traversal-filtered graph output (LLM-facing; focused traversal defaults to depth 2 and direction outgoing); link, status, and representation filters constrain BFS expansion rather than hiding nodes after traversal; Mermaid preserves stored edge orientation and emits deterministic provenance comments. Without --focus, graph show renders the full graph; explicitly supplied traversal flags require `--focus`. `--zones` (requires `--focus`) annotates each node with the directional screen zone it occupies relative to the focus, derived from its direct link to the focus: TOP = what the focus answers to (governs, or refines/extends/grounded-by outgoing from focus); BOTTOM = what builds on the focus (refines/extends/grounded-by/supports incoming); LEFT = tension (contradicts, questions); RIGHT = lateral provenance/task edges (source-of, requires). In `--format json` each node gains a `zone` field (omitted when empty); in `--format text` nodes are grouped under TOP/LEFT/RIGHT/BOTTOM headers. This is the same zone mapping the interactive HTML viewer uses. `--bodies` includes each node's full body inline (and its tags in `--format text`) so a zoned view can be *read* as well as navigated — one call replaces the search→zone→per-node `nn show` loop; in `--format json` each node gains a `body` field (omitted when empty). Combine `--zones --bodies` for a self-contained \"navigator with contents\": neighbors bucketed by zone, each printed with its tags and body.\n\n" +
