...
var virtualGlobalProtocols = []virtualProtocol{
	...
	},
	{
		...
		Body: "**nn new** `--title \"...\" --type <type> --content \"...\" --no-edit [--tags \"t1,t2\"] [--link-to <id> --annotation \"...\"] [--applies-when \"...\"] [--representation <rep>] [--expires YYYY-MM-DD] [--expires-when \"condition\"] [--check]`\n" +
			...
			"**nn ask** `[--surface canvas|document|web] [--instructions \"...\"] [--mermaid \"<diagram>\"] [--document <file|folder>]` — ask a human for feedback via a chosen surface; blocks until they submit, then prints the result path. The result is a thin envelope at `<session-dir>/result.json` naming native artifacts (path-referenced); the agent reads the artifact and decides what, if anything, becomes a note — nothing is filed automatically. Runs without a configured notebook. Session directories are ephemeral: each run reclaims sessions older than 7 days.\n" +
			...
			"  `--surface document` — delegated to the `plannotator` peer for text/markdown annotation; annotates `--document <file|folder>` when given, else the `--instructions` text; writes `result.plannotator.json` (a `{decision, feedback}` object the agent reads).\n\n" +
