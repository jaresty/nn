...
var virtualGlobalProtocols = []virtualProtocol{
	...
	},
	{
		...
		Body: "**nn new** `--title \"...\" --type <type> --content \"...\" --no-edit [--tags \"t1,t2\"] [--link-to <id> --annotation \"...\"] [--applies-when \"...\"] [--representation <rep>] [--expires YYYY-MM-DD] [--expires-when \"condition\"] [--check]`\n" +
			...
			"**nn todo done** `<id> <pattern> [<pattern>...] [--resolution \"why\"]` — mark the first `- [ ]` checkbox whose line contains each <pattern> as done (`- [x]`); multiple patterns flip in a single write (use this instead of parallel `nn todo done` calls, which race and fail the concurrency check); all-or-nothing — if any pattern matches no open checkbox the note is left unchanged; `--resolution` appends commentary explaining why to each flipped line\n" +
			"**nn todo reopen** `<id> <pattern> [<pattern>...]` — mark the first `- [x]` checkbox whose line contains each <pattern> as not done (`- [ ]`); multiple patterns flip in a single write; all-or-nothing — if any pattern matches no done checkbox the note is left unchanged\n" +
