...
var virtualGlobalProtocols = []virtualProtocol{
	...
	},
	{
		...
		Body: "**nn new** `--title \"...\" --type <type> --content \"...\" --no-edit [--tags \"t1,t2\"] [--link-to <id> --annotation \"...\"] [--applies-when \"...\"] [--representation <rep>] [--expires YYYY-MM-DD] [--expires-when \"condition\"] [--check]`\n" +
			...
			"**nn grep** `<pattern> [path] [--context N] [--notes-per-match K] [--max-matches N] [--trace] [--context-report]` — search files for pattern and annotate each match with related nn notes via BM25 on surrounding context; use instead of bare grep when working in a codebase to surface relevant knowledge alongside code locations; `--trace` invokes nn trace inline for each traceable matched file; `--context-report` appends retained source-window counts and exact gross, unique, and overlap line totals\n\n" +
