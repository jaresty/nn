...
var virtualGlobalProtocols = []virtualProtocol{
	...
	},
	{
		...
		Body: "**nn new** `--title \"...\" --type <type> --content \"...\" --no-edit [--tags \"t1,t2\"] [--link-to <id> --annotation \"...\"] [--applies-when \"...\"] [--representation <rep>] [--expires YYYY-MM-DD] [--expires-when \"condition\"] [--check]`\n" +
			...
			"**Conversational navigation history/bookmarks (guide-only; not CLI commands).** Navigation frame = retained focus plus active traversal context and filters. Each successful Teleport, Visit, Recenter, or Go to pushes the prior frame onto Back and clears Forward; an initial landing with no prior/current focus pushes nothing. Back moves current to Forward and restores the latest Back frame; Forward moves current to Back and restores the latest Forward frame. Both rerun Orient and present Focus + Map + Moves. Bookmark names are case-sensitive and save complete frames; replacing one requires explicit confirmation. Go to restores the saved frame as a Teleport landing. Where am I? reports current focus, context/filters, immediate Back/Forward destinations, and bookmark names without mutation. Failed and no-op operations never mutate history or bookmarks; empty Back/Forward and unknown bookmarks report the problem and retain state. State is conversation-scoped. A compaction handoff must carry the complete current frame, ordered Back/Forward stacks, and all named complete bookmark frames; if absent or incomplete afterward, state is unknown; never invent it.\n\n" +
			...
			"**nn graph routes** `nn graph routes --focus ID --links TYPES --search QUERY --limit N --json [--explain]` — discover query-relevant destinations reachable from the focus through only the selected typed, directed links; destination ranking is unchanged and every entry has `destination` plus `witnesses` containing at most 3 shortest node-and-edge paths selected by first-hop diversity, edge type-sequence diversity, then lexical full-path order for Orient, Scan, Peek, Recenter, and Arrive. Without `--explain`, output remains a top-level route array; `--explain` requires `--json` and wraps it as `{routes, diagnostics}` with aggregate `query_tokens`, `total_notes`, `direct_lexical_matches`, `focus_excluded`, `typed_reachable`, `eligible_destinations`, separate annotation-aware `graph_scored_matches`, `returned`, and `truncated_by_limit`—never note content or candidate dumps.\n\n" +
			"**nn graph impact** `nn graph impact --focus ID --links TYPES --direction incoming|outgoing --depth N --json` — return the bounded typed impact set ordered exactly as before. Top-level `summary` contains `total_impacts`, depth-ascending `counts_by_depth`, count-descending-then-ID `counts_by_first_hop` (selected-witness branches may overlap), and honest `witnesses_truncated`; each `node`/`depth` entry still carries unchanged `witnesses` containing at most 3 diverse shortest paths. Outgoing follows stored source→target links, while incoming witness nodes run focus→impact and every edge retains stored source→target orientation for Scan, Peek, Recenter, and Arrive\n\n" +
