...
var virtualGlobalProtocols = []virtualProtocol{
	...
	},
	{
		...
		Body: "**nn new** `--title \"...\" --type <type> --content \"...\" --no-edit [--tags \"t1,t2\"] [--link-to <id> --annotation \"...\"] [--applies-when \"...\"] [--representation <rep>] [--expires YYYY-MM-DD] [--expires-when \"condition\"] [--check]`\n" +
			...
			"**nn graph bridges** `nn graph bridges [--search QUERY] [--format text|json] [--exclude ID] [--limit N]` — return one unified rich bridge model in either encoding. Every result has `id`, `title`, structural `score`, `relevance_score`, and a deterministic `witness` with incoming/outgoing edges plus bounded `regions` context by default: endpoint summaries from full-graph label propagation (`representative` and full `size`) or unclustered, and `same_region`. Without search, relevance is JSON `null` / text `n/a`; search supplies normalized numeric relevance and relevance-first ranking. No durable region IDs are exposed, and `same_region` can be true, so a witness is not proof of territorial separation.\n\n" +
