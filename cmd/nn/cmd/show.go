...
var virtualGlobalProtocols = []virtualProtocol{
	...
	},
	{
		...
		Body: "**nn new** `--title \"...\" --type <type> --content \"...\" --no-edit [--tags \"t1,t2\"] [--link-to <id> --annotation \"...\"] [--applies-when \"...\"] [--representation <rep>] [--expires YYYY-MM-DD] [--expires-when \"condition\"] [--check]`\n" +
			...
			"**nn graph impact** `nn graph impact --focus ID --links TYPES --direction incoming|outgoing --depth N --json` — return the bounded typed impact set ordered exactly as before, with each `node`/`depth` entry carrying `witnesses` containing at most 3 diverse shortest paths; outgoing follows stored source→target links, while incoming witness nodes run focus→impact and every edge retains stored source→target orientation for Scan, Peek, Recenter, and Arrive\n\n" +
