...
func TestNNNavigateAvoidsRedundantTransientRenders(t *testing.T) {
	...
	for _, required := range []string{
		...
		"A completed non-terminal picker selection always reopens that invoking menu",
		"Only when a non-terminal action requested directly in conversation clearly answers that request may it use a quiet return",
