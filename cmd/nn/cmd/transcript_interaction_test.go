...
func TestTranscriptInteractionContract(t *testing.T) {
	...
	for _, term := range []string{"explicit operand", "displayed matching action", "selected target", "rendered", "previous_view", "inspection envelope", "follow-up", "budget", "compaction", "exact restoration unavailable", "empty filter", "capture", "no notebook write", "--format text", "--event", "--payload"} {
...
func TestTranscriptInteractionSingleOwnerRules(t *testing.T) {
	...
	cases := []struct{ name, owner, required, forbidden string }{
		...
		{"BackReplay", "interaction", "**Back is replay, not regeneration.**", "regenerate the prior view"},
