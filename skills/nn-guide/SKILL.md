...
**Notebook-defined aliases.** Search and `--similar` tokenization expands notebook-declared acronyms before stemming. A note declares short forms via an `aliases` frontmatter field (e.g. `aliases: [TDD]` on a note titled "Test-driven development"), set with `nn new --aliases "TDD"` or `nn update <id> --aliases "..."` (replace) / `--aliases-add` / `--aliases-remove` (incremental, like tags); at corpus load, each lowercased alias maps to that note's tokenized title. A query using the alias and a query using the spelled-out title then converge on the same tokens, closing the acronym gap that stemming alone cannot bridge (`TDD` shares no morpheme with `test`/`driven`/`development`). Aliases are notebook truth, versioned in Git, and rebuilt with the index — no separate config. Declaring the same case-folded alias on two different notes is a conflict; the conflicting table is not installed (search falls back to unexpanded tokens) rather than breaking listing.
...
`nn-navigate` owns the detailed zoned navigation model, arrival scaling, route/impact/path overlays, teleport/scan/peek behavior, contextual ✚ Integrate proposals for bounded notebook edits, chooser and presentation discipline, its colon-prefixed conversational DSL with Guided/Advanced interaction, and conversation-scoped history, bookmarks, and compaction. The DSL is skill-level intent, not a set of `nn` subcommands; load the owner for its grammar, mode, label-resolution, and return rules. Its compact core dispatches those actions to lazy references. Before an applicable action, follow the core's ownership table and fetch each owner with `nn skills get nn-navigate --reference <name>`; use `--list-references` to inspect the stable names and applicability. nn-navigate owns Find an analog and the optional Analogize, Find gaps, Visualize, and Quiz lenses. Do not conduct the iterative workflow from this command reference alone or treat the compact core as inlined action detail.
...
- `--surface document` (*delegated*) — hands the document to the `plannotator` peer for text/markdown annotation. Annotates `--document <file|folder|url>` when given (a `https://` URL is passed straight through — plannotator fetches it), otherwise the `--instructions` text. Local `.html`/`.htm` files render as raw pages with a session-scoped Plannotator data directory, avoiding large historical HTML diffs that can blank Plannotator 0.27.8; this intentionally disables cross-session Plannotator version history only for those local HTML reviews. Writes `result.plannotator.json`, a `{ "decision": "approved|annotated|dismissed", "feedback": "..." }` object you read to get the human's annotations.
