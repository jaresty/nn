...
**The wider landscape (global)** — where your region sits in everything (drops the ego as the organizing center, but may still mark its verified location). Before a Global landscape marks the retained focus as `YOU`, run both exact structural lookups:
...
This exact focus-ID anchor must complete before any freeform `--search` projection. It establishes the focus's full-graph cluster membership or policy omission and its exact bridge/non-bridge status. When the bridge envelope is non-null, inspect its witnesses: `same_region: true` is not evidence of a cross-region crossing, and bridge score is a connector heuristic rather than an articulation-point proof.

Freeform search belongs at this Global landscape altitude only after that anchor. Freeform search ranks global relevance; it never establishes the focus's cluster membership or bridge status. Use query-conditioned cluster and bridge results to decide which other regions and highways matter to the retained goal, then place the already-verified `YOU` marker into that projected landscape:
