...
## `:look` — redraw the retained positioned view

`:look` is a non-mutating full-location query. Fetch `state` and `presentation`, verify that the complete retained frame is available, then retrieve topology and complete bodies before rendering `:look` with the frame's identical focus and traversal filters. Redraw Focus + Map + Moves without pushing Back, clearing Forward, changing focus, altering filters or traversal context, changing bookmarks, or changing interaction mode. If the retained frame is missing or incomplete, report location state unknown rather than reconstructing it. Use `:where` for a compact focus/context/history/bookmark summary; use `:orient` when the human asks to recompute orientation or refresh the walk's decision framing.
