...
- **ResultAvailable → Positioned:** report the human finding as evidence and restore the byte-for-byte-equivalent frame and UI values. In Guided mode, reopen the invoking Ask submenu; if Ask was a promoted top-level shortcut, return to Quick actions instead. In Advanced mode, retain that semantic menu position but keep Navigation help closed. Cancellation follows this same return path and mode-aware menu rule with no state mutation.
