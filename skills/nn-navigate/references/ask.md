...
bookmarks: {}
visited_evidence: []
interaction_mode: guided | advanced
current_menu: Ask
menu_stack: []
```
...
- **Positioned → AskPrepared:** state one decision the human can change, include only bounded visited evidence, select the cheapest faithful surface, and retain interaction mode, the invoking menu, and its ordered menu stack.
...
- **ResultAvailable → Positioned:** report the human finding as evidence and restore the byte-for-byte-equivalent frame and UI values. In Guided mode, reopen the invoking Ask submenu when another consultation choice is naturally pending; if the completed Ask directly and clearly answers the explicit human intent, use `presentation`'s quiet return while retaining that Ask menu position. A promoted top-level Ask returns semantically to Quick actions under the same rule. In Advanced mode, retain that semantic menu position but keep Navigation help closed. Cancellation follows the ordinary reopen path with no state mutation.

Ask does not change focus; does not push Back or clear Forward; does not modify bookmarks, filters, traversal context, visited evidence, or interaction mode automatically; does not create or edit notes or links; and does not turn a selected note into an implicit Recenter. It must preserve interaction mode through preparation, waiting, completion, cancellation, and any Graph handoff, and must not persist navigation/menu UI state in notebook files, frontmatter, the index, Git, configuration, or environment. A result can justify a later Recenter, Peek, ✚ Integrate mutation proposal, or Arrive only as a separately proposed action under that action's normal contract; every notebook edit dispatches to `integrate`.
...
- **Graph — Review or group this graph material:** use the retained focus and an explicit bounded node allowlist. The graph-native result has exactly `groups`, `overall_comment`, and `handoff` at top level; note and edge comments live on members inside groups. The strict decoder accepts exactly `null`, `canvas`, or `document` and strictly rejects obsolete `handoffs` and `explain_on_canvas`, unknown top-level keys, and every other handoff value. The graph answer may contain multiple saved, optionally named groups with group comments and lightweight feedback classifications. The composer visibly marks any changed active selection, name, classification, or group comment as **Unsaved group**. Every terminal action automatically saves a pending group that has selected note or relationship membership before constructing the canonical payload. A non-empty pending group without members blocks submission and visibly asks the human to select at least one note or relationship; no terminal action silently discards composer text. Modifier-click adds/removes notes or relationships; lasso, when available, identifies members rather than durable geometry. A lasso directly catches every node whose rendered circle intersects its rectangle. It directly catches an edge exactly when the rectangle contains the rendered path midpoint or intersects the visible relationship label; a relationship segment merely crossing elsewhere is not directly selected. Each group records node and edge membership. Mark direct lasso hits `selection: explicit`. An unchosen connecting edge between explicitly lassoed nodes is `selection: implicit` unless its midpoint or label was directly caught; a directly caught edge pulls any endpoint outside the rectangle into the group as an implicit node. This distinguishes explicitly selected edges from edges included implicitly, keeps relationships interpretable, and preserves direct-versus-derived intent. Classifications such as `relevant`, `evidence`, `tension`, `belong-together`, `needs-attention`, and `unclear` are feedback labels, never notebook link types.
- **Canvas — Explain or critique these concepts:** seed sections, claims, evidence, assumptions, and implications within a note, or selected notes/stored edges/named Graph groups across notes. Clearly distinguish copied stored-edge references from explanatory arrows, containment, ordering, proximity, and grouping. Every seeded explanatory diagram has exactly one unobtrusive diagram-level explanatory note with this text: **Illustrative layout and inferred relationships — not literal notebook structure. Source IDs identify evidence; only edges explicitly marked STORED are notebook edges.** Mark copied notebook edges `STORED`, and do not repeat `NON_STORED` on each node or relation; human edits to Canvas are evidence, while generated seed geometry remains agent material and neither is notebook meaning.
...
Graph Ask offers three mutually exclusive terminal buttons: **Send** → `handoff: null`; **Send to Canvas** → `handoff: canvas`; **Send to Document** → `handoff: document`. There is no persistent toggle or fallback state. Clicking any one first applies the pending-group rule above, then submits exactly once and closes the Graph Ask surface. The graph-selection artifact remains authoritative, with exactly `groups`, `overall_comment`, and `handoff` at top level. Graph emits a canvas-seed artifact only for `canvas`, labeled `NON_STORED`. Document has no separate seed artifact because the later temporary brief supplies the needed context without artifact proliferation. The Graph server and the `nn ask` CLI MUST NOT launch Canvas or Document directly.
...
6. After the synchronous Ask completes, read the returned Canvas artifact, compare the generated seed baseline with the returned Canvas artifact, and report every human-added or human-edited annotation whose origin is evidenced—for example, new text absent from the Mermaid source—as human evidence. Distinguish generated seed material from evidenced human changes. Because the current artifact contract retains Mermaid source rather than a native converted-scene baseline, label geometry-only additions, deletions, movement, binding, or regrouping **attribution uncertain** unless their human origin is otherwise evidenced; never attribute unchanged generated geometry to the human, and never dismiss an annotation because its meaning is unclear. Ask for clarification when semantics or authorship are ambiguous rather than inventing or downgrading intent. Then restore the exact snapshotted frame, including focus, goal, filters and traversal context, Back, Forward, bookmarks, visited evidence, interaction mode, current menu, and ordered menu stack. In Guided mode, use the ordinary invoking-menu return when another choice is pending and the quiet return when this explicit direct intent is clearly complete; in Advanced mode keep Navigation help closed.
...
5. Remove the temporary brief when no longer needed, then restore the exact snapshotted frame, including focus, goal, filters and traversal context, Back, Forward, bookmarks, visited evidence, interaction mode, current menu, and ordered menu stack. In Guided mode, use the ordinary invoking-menu return when another choice is pending and the quiet return when this explicit direct intent is clearly complete; in Advanced mode keep Navigation help closed.
