...
- **Where am I? / `:where`** is the compact location query: report the current focus, active traversal context and filters, immediate Back and Forward destinations (if any), and bookmark names. It does not redraw Focus + Map + Moves and does not mutate navigation state. Use `:look` for the complete retained positioned view.
