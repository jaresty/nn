...
Only **✚ Integrate** owns Navigation-discovered notebook mutation; fetch `nn skills get nn-navigate --reference integrate`. It proposes before writing, requires approval, preserves specialized safeguards, and never treats generated material as stored truth. Natural requests such as “integrate this media run RUN_ID” dispatch here: retrieve every bounded page with `nn media context --run RUN_ID`, use available image-reading capability to load every typed image attachment, treat transcript and images as evidence, search and read existing notebook truth, then produce the ordinary non-mutating proposal. This is not an `nn integrate` command. Other actions may suggest changes but remain non-mutating. At every seam: retain state, fetch ownership, read bounded evidence, present after validation, and mutate only under the explicit owner.
